import {
  ORGS, PROJECTS, INVESTORS,
  getDashboard, getImpactoResumo, getMonitoramentoConsolidado,
  getMonitoramentoProjeto, getOrgScore,
} from './mockData';
...
export const api = {
  ...
  organizacoes: (params?: Record<string, string>) => {
    ...
    if (params?.ods)   data = data.filter(o => o.ods.includes(Number(params.ods) as any));
  ...
  projetos: (params?: Record<string, string>) => {
    ...
    if (params?.ods)    data = data.filter(p => p.ods.includes(Number(params.ods) as any));
    return asPromise({ total: data.length, data });
  },
  ...
  matchInvestidor: (id: string) => {
    ...
    return asPromise({ investidorId: id, totalMatches: matches.length, projetos: matches });
  },
